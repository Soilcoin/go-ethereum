...
import (
	...
	"github.com/Soilcoin/go-soil/consensus"
	"github.com/Soilcoin/go-soil/consensus/misc"
	"github.com/Soilcoin/go-soil/core"
	"github.com/Soilcoin/go-soil/core/types"
	"github.com/Soilcoin/go-soil/eth/downloader"
	"github.com/Soilcoin/go-soil/eth/fetcher"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Soil peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Soil protocol")
	...
	log.Info("Soil protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Soil peer connected", "name", p.Name())

	// Execute the Ethereum handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("Soil handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Soil peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Soil message handling failed", "err", err)
			return err
		}
	}
}
